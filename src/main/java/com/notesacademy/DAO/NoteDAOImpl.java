...
//    --------------------------------------------------- Displaying notes in the main website based on the subject id -----------------------------------------
       ...
       Note n = null;
        ...
        try 
        {
            ...
            while(rs.next())
            {
                ...
                n.setLikeCount(rs.getInt(14));
                n.setUserId(rs.getInt(15));
                n.setNotePrice(rs.getInt(16));
                n.setNoteRazor(rs.getString(17));
                n.setDownloadCount(rs.getInt(18));
                n.setViewCount(rs.getInt(19));
                n.setUserRank(rs.getInt(20));
    ...
    public boolean sendNotes(Note n) 
    {
        ...
        try
        {
         String sql = "INSERT INTO tempnotes(noteTitle,noteDescription,categoryName,courseName,subjectYear,subjectName,noteDate,userId,userName,userProfession,userCollege,pdfPath,subject_id,notePrice,noteRazor,userRank) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; 
    ...
    public boolean addNote(Note n) 
    {
        ...
        try
        {
         String sql = "INSERT INTO note(noteTitle,noteDescription,categoryName,courseName,subjectYear,subjectName,noteDate,userName,userProfession,userCollege,pdfPath,subject_id,userId,notePrice,noteRazor,userRank) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"; 
...
//  ----------------------------------------  Get details of main notes from note table based on noteid ----------------------------------------------
    ...
    public boolean updateEditNote(Note n) 
    {
       ...
       try
       {
           String sql = "UPDATE note SET noteTitle=?,noteDescription=?,categoryName=?,courseName=?,subjectYear=?,subjectName=?,subject_id=?,notePrice=?,noteRazor=? WHERE noteId=?";
           PreparedStatement ps = con.prepareStatement(sql);
           ...
           ps.setString(5,n.getSubjectYear());
           ps.setString(6,n.getSubjectName());
           ps.setInt(7,n.getSubject_id());
           ps.setInt(8,n.getNotePrice());
           ps.setString(9,n.getNoteRazor());
           ps.setInt(10,n.getNoteId());
    ...
    public List<Note> getMostLikedSearchNotes(String ch) 
    {
     ...
     // ---------------------------------------------- Displaying all notes of a particular user ------------------------------------------
    ...
    public List<Note> getNotesByUserId(int id)
    {
        ...
        try 
        {
            String sql = "SELECT * FROM note WHERE userId = ? ORDER BY noteId DESC";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1,id);
            ResultSet rs = ps.executeQuery();
        ...
        {
            System.out.println("There is error in NoteDAOImpl - getNotesByUserId : "+e);
        }
    ...
    // ---------------------------------------- Displaying all notes whose views are less than 10 ---------------------------------------
    ...
    public List<Note> getUnusedNotes() 
    {
        ...
        try 
        {
            String sql = "SELECT * FROM note WHERE viewCount<=2";
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
        ...
        {
            System.out.println("There is error in NoteDAOImpl - getUnusedNotes : "+e);
        }
    ...
    //---------------------------------------------- Delete note from note table -----------------------------------------
    
    @Override
    public boolean deleteMainNote(int id) 
    {
        boolean f = false;
        ...
        {
            System.out.println("There is error in NoteDAOImpl - deleteMainNote : "+e);
        }
