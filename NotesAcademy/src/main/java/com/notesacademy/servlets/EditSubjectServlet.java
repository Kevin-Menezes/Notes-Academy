...
public class EditSubjectServlet extends HttpServlet {
            ...
            throws ServletException, IOException 
    {
        try 
        {
            ...
            boolean f = dao.updateEditSubject(s);
            
            if(f)
            {
                Message msg = new Message("Subject edited successfully!", "success", "alert-success");
                session.setAttribute("message", msg);  
                response.sendRedirect("display_subjects.jsp"); // DIRECT IT TO THE USERS HOMEPAGE
            }
            else
            {
